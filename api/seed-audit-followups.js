// /api/seed-audit-followups.js
// One-off: generates + schedules follow-up sequences for all delivered lead audits
// that don't have follow-ups yet. Calls the same email builders as send-audit-email.js.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    for (var i = 0; i < (audits || []).length; i++) {
      ...
      if (!contact) continue;
      // Only leads/prospects
      if (contact.status !== 'lead' && contact.status !== 'prospect') continue;

      // Check for existing followups
      var existing = await sb.query('audit_followups?audit_id=eq.' + audit.id + '&limit=1');
      if (existing && existing.length > 0) {
        results.push({ slug: contact.slug, status: 'skipped', reason: 'already has followups' });
        continue;
      }
      ...
      var now = new Date();
      var rows = fuEmails.map(function(e, idx) {
      ...
      results.push({ slug: contact.slug, status: 'created', count: rows.length });
    }
...
// ── Email builders (same as send-audit-email.js) ──
...
function wrapFollowup(content) {
  return email.wrap({ headerLabel: 'CORE Entity Audit', content: content, footerNote: FOOTER_NOTE, year: new Date().getFullYear() });
}
...
function buildFollowupSequence(audit, contact) {
  ...
  // Handle tasks being either an array or an object
...
function buildEmail2(firstName, practiceName, weakest, secondWeakest, weakTasks, secondTasks, scorecardUrl, bookingUrl) {
  ...
  if (weakTasks.length > 0 || secondTasks.length > 0) {
    ...
    if (weakTasks.length > 0) {
      weakTasks.forEach(function(t) { findingsHtml += email.p('&bull; ' + email.esc(t.title || t.description || '')); });
    } else {
    ...
    if (secondTasks.length > 0) {
      ...
      secondTasks.forEach(function(t) { findingsHtml += email.p('&bull; ' + email.esc(t.title || t.description || '')); });
    }
  }
  var content = email.greeting(firstName || 'there') +
