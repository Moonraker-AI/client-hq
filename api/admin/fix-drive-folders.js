...
var DEREK_PARENT_ID = '1m49iH0CcCkDWo3yeltnC9U_slhUPdCBn';  // Roaming Rehab parent folder
...
async function fixDerek(H) {
  ...
  // 1. Check Roaming Rehab parent for any existing non-trashed Creative folder
  var existingResp = await fetch(DRIVE_API + '/files?q=' +
    encodeURIComponent("'" + DEREK_PARENT_ID + "' in parents and trashed = false and mimeType = 'application/vnd.google-apps.folder' and name = 'Creative'") +
  ...
  var existing = (existingData.files || []);

  var creativeId;
  if (existing.length > 0) {
    creativeId = existing[0].id;
    log.push('found existing non-trashed Creative folder: ' + creativeId);
  } else {
    var c = await fetch(DRIVE_API + '/files?supportsAllDrives=true', {
      ...
      body: JSON.stringify({
        ...
        parents: [DEREK_PARENT_ID]
      })
    });
    if (!c.ok) throw new Error('create Creative failed: ' + c.status + ' ' + (await c.text()));
    var cdata = await c.json();
    creativeId = cdata.id;
    log.push('created fresh Creative folder: ' + creativeId);
  }

  // 2. List existing subfolders inside Creative
  var kidsResp = await fetch(DRIVE_API + '/files?q=' +
    encodeURIComponent("'" + creativeId + "' in parents and trashed = false and mimeType = 'application/vnd.google-apps.folder'") +
    '&fields=files(id,name)&pageSize=50&supportsAllDrives=true&includeItemsFromAllDrives=true',
    { headers: H });
  var kidsData = await kidsResp.json();
  var existingNames = (kidsData.files || []).map(f => f.name);
  ...
  for (var i = 0; i < DEFAULT_CREATIVE_CHILDREN.length; i++) {
    ...
    var r = await fetch(DRIVE_API + '/files?supportsAllDrives=true', {
      method: 'POST', headers: H,
      body: JSON.stringify({
        name: name,
        mimeType: 'application/vnd.google-apps.folder',
    ...
    createdChildren.push(name);
    log.push('created ' + name + ' (' + rdata.id + ')');
  }

  // 3. Update DB to point to new Creative
  await sb.mutate('contacts?id=eq.' + DEREK_CONTACT_ID, 'PATCH', {
  ...
  return {
    ...
    action: 'derek',
    log: log,
    new_creative_folder_id: creativeId,
    created_children: createdChildren,
    note: 'Trashed old Creative and 4 loose cert files left in place. Manual cleanup from Drive UI.'
  };
}
